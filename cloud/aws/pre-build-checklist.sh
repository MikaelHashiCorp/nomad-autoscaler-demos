...
# This script MUST be run and completed before any build deployment

set -e

BUILD_NUMBER=${1:-"UNKNOWN"}
CHECKLIST_FILE="PRE_BUILD_${BUILD_NUMBER}_CHECKLIST.md"

echo "============================================"
echo "  PRE-BUILD DUE DILIGENCE CHECKLIST"
...
sed -i.bak "s/BUILD_NUMBER_PLACEHOLDER/$BUILD_NUMBER/g" "$CHECKLIST_FILE"
sed -i.bak "s/DATE_PLACEHOLDER/$(date -u +"%Y-%m-%d %H:%M UTC")/g" "$CHECKLIST_FILE"
sed -i.bak "s/REVIEWER_PLACEHOLDER/$(whoami)/g" "$CHECKLIST_FILE"
rm "${CHECKLIST_FILE}.bak" 2>/dev/null || true

echo "✅ Checklist created: $CHECKLIST_FILE"
echo ""
echo "MANDATORY STEPS:"
echo "1. Open and complete: $CHECKLIST_FILE"
echo "2. Mark ALL checkboxes [ ] as [x]"
echo "3. Fill in ALL sections"
echo "4. Sign off at the bottom"
echo "5. Only then run: terraform apply"
echo ""
echo "⚠️  DO NOT SKIP THIS PROCESS ⚠️"
echo ""
...
if command -v code &> /dev/null; then
    ...
    echo "Please open and complete: $CHECKLIST_FILE"
fi
